...
function updateColumnMetadata(weave, path, diff)
{
	var meta = getColumnMetadata(weave, path);
	console.log( meta );
	for (var k in diff)
		meta[k] = diff[k];
	setColumnMetadata(weave, path, meta, "WeaveDataSource");
}

function getColumnMetadata(weave, path, names)
{
	if (!names)
		names = ['baseTitle', 'weaveEntityId', 'sqlParams'];
	var meta = weave.path(path).getValue("Class('weave.api.data.ColumnMetadata').getAllMetadata(this)");
	var result = {};
	for (var i in names)
		result[names[i]] = meta[names[i]];
	return result;
}

function setColumnMetadata(weave, path, metadata, dataSourceName)
{
    // make sure path refers to a DynamicColumn, create a ReferencedColumn inside the DynamicColumn, and set the column reference
    var wp = weave.path(path);
	wp.request('DynamicColumn')
		.push(null)
			.request('ReferencedColumn')
			.state({
				"dataSourceName": dataSourceName,
...
function removePathChildren( weave, path )
{
	var wp = weave.path( path ),
		names = wp.getNames();
...
function getEntityObjectByMetadataPath( tree, path )
{
	if( tree == null )
		return null;
	...
	var i = 0, children = tree.children;
	for( ; i < children.length; i++ )
	{
		...
		for( var key in meta ) {
			if( thisChild.publicMetadata[key] != meta[key] ) {
				nomatch = true;
				continue;
			}
		}
		if( nomatch ) continue;
		return getEntityObjectByMetadataPath( thisChild, path.slice(1) );
	}
	return null;
}

function getEntityObjectByTreePath( tree, path )
{
	return getEntityObjectByMetadataPath( tree, path );
}

function getEntityIdByTreePath( tree, path )
{
	var obj = getEntityObjectByTreePath( tree, path );
	
	return ( ( obj != null ) ? obj.id : null );
}

function getTreePathByEntityId(tree, id) 
{
	if (tree == null || id <= 0)
		return null;
	...
	for (; i < children.length; i++) 
	{
		...
		var childPath = getTreePathByEntityId(thisChild, id);

		if (childPath == null)
			continue;

		childPath.unshift(thisChild.publicMetadata.title);
...
function getSiblingEntityIds( tree, id, includeSelf )
{
	var i = 0,
		path = getTreePathByEntityId( tree, id ),
	...
	path.pop();
	parent = getEntityObjectByTreePath( tree, path );
	children = parent.children;
	
	for(; i < children.length; i++ )
	{
		var child = children[i];
		if( child.id != id || includeSelf )
